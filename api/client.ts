...
export const api = {
  ...
  backup: {
    export: async () => {
      ...
      a.href = url
      a.download = `northstar-${new Date().toISOString().slice(0, 10)}.csv`
      a.click()
      URL.revokeObjectURL(url)
    },
    import: (csvText: string) =>
      request<{ imported: { properties: number; monthlyData: number } }>(
        '/backup/import',
        { method: 'POST', body: csvText, headers: { 'Content-Type': 'text/csv' } }
      ),
  },
}
