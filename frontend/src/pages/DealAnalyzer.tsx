...
import { PropertyType, Deal, FinancingType } from '@northstar/shared-types'
        ...
        {isConverted ? (
          <div className="flex items-center gap-2">
            ...
            {deal.convertedPropertyId && (
              <button onClick={() => navigate(`/properties/${deal.convertedPropertyId}`)} className="text-xs text-brand-600 hover:underline whitespace-nowrap">
                View →
              </button>
            )}
          </div>
...
function CumulativeReturnChart({ projections }: { projections: YearProjection[] }) {
  const W = 560, H = 148
  const padL = 68, padR = 16, padT = 28, padB = 22
  const chartW = W - padL - padR
  const chartH = H - padT - padB

  const values = projections.map(p => p.cumulativeReturn)
  const minVal = Math.min(0, ...values)
  const maxVal = Math.max(1, ...values)
  const span = maxVal - minVal || 1

  const xOf = (year: number) => padL + ((year - 1) / 19) * chartW
  const yOf = (val: number) => padT + chartH - ((val - minVal) / span) * chartH

  const zeroY = yOf(0)
  const linePts = projections.map(p => `${xOf(p.year).toFixed(1)},${yOf(p.cumulativeReturn).toFixed(1)}`).join(' ')
  const areaPts = [
    `${xOf(1).toFixed(1)},${zeroY.toFixed(1)}`,
    ...projections.map(p => `${xOf(p.year).toFixed(1)},${yOf(p.cumulativeReturn).toFixed(1)}`),
    `${xOf(20).toFixed(1)},${zeroY.toFixed(1)}`,
  ].join(' ')

  const yStops = [0, 0.333, 0.667, 1].map(t => minVal + t * span)
  const milestones = [5, 10, 15, 20]

  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ maxHeight: H }}>
      <defs>
        <linearGradient id="cumLineGrad" x1="0" y1="0" x2="0" y2="1">
      ...
      {yStops.map((val, i) => (
        <g key={i}>
          <line x1={padL} y1={yOf(val)} x2={W - padR} y2={yOf(val)} stroke="#f3f4f6" strokeWidth="1" />
          <text x={padL - 5} y={yOf(val) + 3.5} textAnchor="end" fontSize="8" fill="#9ca3af">
            {formatCurrency(val, true)}
          </text>
        </g>
      ))}
      {/* Zero line */}
      <line x1={padL} y1={zeroY} x2={W - padR} y2={zeroY} stroke="#d1d5db" strokeWidth="1" />
      {/* Area */}
      <polygon points={areaPts} fill="url(#cumLineGrad)" />
      {/* Line */}
      <polyline points={linePts} fill="none" stroke="#10b981" strokeWidth="2" strokeLinejoin="round" strokeLinecap="round" />
      {/* Milestone dots + labels */}
      {milestones.map(yr => {
        const p = projections[yr - 1]
        const cx = xOf(yr), cy = yOf(p.cumulativeReturn)
        const isPos = p.cumulativeReturn >= 0
        return (
          <g key={yr}>
            <circle cx={cx} cy={cy} r="3.5" fill="#10b981" stroke="white" strokeWidth="1.5" />
            <text x={cx} y={cy - 7} textAnchor="middle" fontSize="8.5" fill={isPos ? '#059669' : '#dc2626'} fontWeight="600">
      ...
      {/* X-axis labels */}
      {[1, 5, 10, 15, 20].map(yr => (
        <text key={yr} x={xOf(yr)} y={H - 4} textAnchor="middle" fontSize="8" fill="#9ca3af">Yr {yr}</text>
      ))}
    </svg>
  )
}
...
function ProjectionSection({ inputs, result }: { inputs: ProFormaInputs; result: ProFormaResult }) {
  ...
  const yr5 = projections[4]
  const yr20 = projections[19]
  const chartYears = projections.slice(0, 5)

  const maxBarValue = Math.max(1, ...chartYears.map(p =>
    Math.max(0, p.annualCashFlow) + p.principalPaydown + p.appreciationGain
  ))

  function barH(val: number) {
    return Math.round(Math.max(0, val) / maxBarValue * CHART_H)
  }
  ...
  return (
    <div className="mt-6 bg-white rounded-2xl shadow-card overflow-hidden">
      ...
      {/* Summary stats — 5yr and 20yr side by side */}
      <div className="px-5 pt-4 pb-4 grid grid-cols-2 sm:grid-cols-4 gap-4 border-b border-gray-100">
        {[
          { label: '5-Yr Total Return', value: formatCurrency(yr5.cumulativeReturn, true), color: yr5.cumulativeReturn >= 0 ? 'text-gray-900' : 'text-red-600' },
          { label: '5-Yr ROI',          value: `${yr5.roi.toFixed(1)}%`,                   color: yr5.roi >= 0 ? 'text-emerald-700' : 'text-red-600' },
          { label: '20-Yr Total Return',value: formatCurrency(yr20.cumulativeReturn, true), color: yr20.cumulativeReturn >= 0 ? 'text-gray-900' : 'text-red-600' },
      ...
      <div className="p-5">
        {/* Chart — 5 years */}
        <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-3">5-Year Chart</p>
        <div className="flex gap-3 mb-3">
          {chartYears.map(p => {
            const cfH = barH(p.annualCashFlow)
            const ppH = barH(p.principalPaydown)
            const apH = barH(p.appreciationGain)
            const totalH = cfH + ppH + apH
            return (
              <div key={p.year} className="flex-1 flex flex-col items-center">
                <span className="text-[11px] font-bold text-gray-600 mb-1">{p.roi.toFixed(0)}%</span>
                <div className="relative w-full" style={{ height: CHART_H }}>
                  <div
                    className="absolute bottom-0 left-0 right-0 flex flex-col-reverse rounded-t-sm overflow-hidden"
                    style={{ height: totalH }}
                  >
                    <div style={{ height: cfH }} className={p.annualCashFlow >= 0 ? 'bg-emerald-400 shrink-0' : 'bg-red-400 shrink-0'} />
                    {isAmortizing && <div style={{ height: ppH }} className="bg-blue-400 shrink-0" />}
                    <div style={{ height: apH }} className="bg-violet-400 shrink-0" />
                  </div>
                </div>
                <span className="text-[11px] text-gray-400 mt-1.5">Yr {p.year}</span>
              </div>
            )
          })}
        </div>
        ...
        <div className="mb-6">
          <CumulativeReturnChart projections={projections} />
        </div>

        {/* Table — 20 years */}
        <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-3">20-Year Table</p>
